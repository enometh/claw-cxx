...
;; SDL_GetTicks() now returns a 64-bit value.
;; Uint64 deadline = SDL_GetTicks() + 1000
;;if (SDL_GetTicks() >= deadline) {    ... }

(defmacro get-ticks ()
  "Returns an unsigned 64-bit value representing the number of milliseconds since the SDL library
initialized."
  `(sdl-get-ticks))
...
(defun remove-timer (timer)
  ...
  (check-rc (sdl-remove-timer timer)))
