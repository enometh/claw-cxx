...
;;madhu 250326 -- mupdf-25.1 from enometh-overlay, test with a
;; different source location
(in-package "CL-USER")
(defparameter *cl-claw-mupdf-source-dir* "/dev/shm/cl-claw-mupdf/")
(defparameter *cl-claw-mupdf-binary-dir*
  (let ((user::*binary-directory-fasl-root* "/dev/shm/cl-claw-mupdf/fasl/"))
    (binary-directory
     "/dev/shm/cl-claw-mupdf/`")))
...
(require 'claw-cxx))
;; target
(load (merge-pathnames "cl-claw-mupdf.system" *cl-claw-mupdf-source-dir*))
;; (load "~/cl/extern/cl-claw-mupdf/cl-claw-mupdf")
(mk:oos :cl-claw-mupdf :load)

;; force the generatation of bindings gen/bindings/x86_64-pc-linux-gnu.lisp
...
(load (merge-pathnames "claw-cxx-mupdf.system" *cl-claw-mupdf-source-dir*))
...
(mk:oos :cl-claw-mupdf.bindings :load)
(mk:oos :cl-claw-mupdf.library :load)
...
* static-inline hack
Work around static inline definitions which dont get picked up by
claw.  Do this redefining static and inline to nop when processing
resect, the adapter generates functions for the definitions.

- mymupdf.h :: include mupdf/fitz.h without respecting static inlines
- wrapper.lisp :: include mymupdf.h instead of mupdf/fitz.h

#+begin_src :eval no
# ;madhu 250326
  gcc -I$. -DHAVE_LEPTONICA -DHAVE_TESSERACT -c mupdf-static-inlines.c -fPIC -DPIC --no-inline
  gcc -o mupdf-static-inlines.so -shared mupdf-static-inlines.o -lm -Wl,--export-dynamic
  objdump -t mupdf-static-inlines.so
#+end_src

* Example: extract pdf links
#+begin_src lisp
;;; load claw-mupdf-user
;madhu 250326
(require 'claw-cxx/util)
(load (merge-pathnames "claw-cxx-mupdf.system" *cl-claw-mupdf-source-dir*))
(load (merge-pathnames "cl-claw-mupdf-user.system" *cl-claw-mupdf-source-dir*))
(mk:oos :cl-claw-mupdf-user :load)
(cl-claw-mupdf::extract-links "/dev/shm/1.pdf" :external-links-only-p t)
#+end_src
