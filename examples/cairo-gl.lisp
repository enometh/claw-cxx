...
(defun cairo-test (&key (width *texture-size*) (height *texture-size*)
  ...
  (sdl3:with-init (:everything)
    (multiple-value-bind (window renderer)
        (sdl3:create-window-and-renderer width height '(:opengl))
      (sdl3:with-gl-context (gl window)
        (sdl3:gl-make-current window gl)
        (gl:enable :texture-2d)
        (gl-ortho-setup :width width :height height)
	(cffi:with-foreign-object (tex :unsigned-char (* *texture-size* *texture-size* 4))
	      ...
	      #+nil
              (:key-up () (sdl3:push-event :sdl-event-quit))
	      #+nil
              (:sdl-event-key-down () (sdl3:push-event :sdl-event-quit))
              (:sdl-event-mouse-button-down () (sdl3:push-event :sdl-event-quit))
