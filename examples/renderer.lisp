;madhu 250605 --  cl-sdl2/examples/renderer.lisp updated for cl-claw-sdl3
(in-package #:cl-claw-sdl3-examples)

(defun test-render-clear (renderer)
  (sdl3:set-render-draw-color renderer 0 0 0 255)
  (sdl3:render-clear renderer))

(defun test-render-hello (renderer)
  (sdl3:set-render-draw-color renderer 255 0 0 255)
  ;; H
  (sdl3:render-draw-line renderer 20 20 20 100)
  (sdl3:render-draw-line renderer 20 60 60 60)
  (sdl3:render-draw-line renderer 60 20 60 100)
  ;; E
  (sdl3:render-draw-line renderer 80 20 80 100)
  (sdl3:render-draw-line renderer 80 20 120 20)
  (sdl3:render-draw-line renderer 80 60 120 60)
  (sdl3:render-draw-line renderer 80 100 120 100)
  ;; L
  (sdl3:render-draw-line renderer 140 20 140 100)
  (sdl3:render-draw-line renderer 140 100 180 100)
  ;; L
  (sdl3:render-draw-line renderer 200 20 200 100)
  (sdl3:render-draw-line renderer 200 100 240 100)
  ;; O
  (sdl3:render-draw-line renderer 260 20 260 100)
  (sdl3:render-draw-line renderer 260 100 300 100)
  (sdl3:render-draw-line renderer 300 20 300 100)
  (sdl3:render-draw-line renderer 260 20 300 20))

(defun test-render-lines (renderer)
  (sdl3:with-points ((a 200 200)
                     (b 300 400)
                     (c 400 200))
    (sdl3:set-render-draw-color renderer 0 0 255 255)
    (multiple-value-bind (points num) (sdl3:points* a b c)
      (sdl3:render-draw-lines renderer points num))))

(defun test-render-points (renderer)
  (sdl3:with-points ((a (random 800) (random 800))
                     (b (random 800) (random 800))
                     (c (random 800) (random 800)))
    (sdl3:set-render-draw-color renderer 0 255 0 255)
    (multiple-value-bind (points num) (sdl3:points* a b c)
      (sdl3:render-draw-points renderer points num))))

(defun test-render-rect (renderer)
  (sdl3:render-draw-rect renderer (sdl3:make-rect 400 400 35 35)))

(defun test-render-rects (renderer)
  (multiple-value-bind (rects num)
      (apply #'sdl3:rects*
             (loop :for x :upto 5
                   :for y :upto 5
                   :collect (sdl3:make-rect (+ 400 (* x 10)) (+ 200 (* y 10)) 8 8)))
    (sdl3:render-draw-rects renderer rects num)))

(defun test-render-fill-rect (renderer)
  (sdl3:render-fill-rect renderer (sdl3:make-rect 445 400 35 35)))

(defun test-render-fill-rects (renderer)
  (multiple-value-bind (rects num)
      (apply #'sdl3:rects*
             (loop :for x :upto 5
                   :collect (sdl3:make-rect (+ 500 (* x 10)) 400 8 8)))
    (sdl3:set-render-draw-color renderer 255 0 255 255)
    (sdl3:render-fill-rects renderer rects num)))

(defun renderer-test ()
  "Test the SDL_render.h API"
  (sdl3:with-init (:everything)
    (sdl3:with-window (win :title "SDL3 Renderer API Demo" :flags '(:opengl))
      (sdl3:with-renderer (renderer win :name ;; "software"
				    "opengl")
        (sdl3:with-event-loop (:method :poll)
          (:key-up
           (:scancode scancode)
           (when (eql scancode :scancode-escape)
             (sdl3:push-event :sdl-event-quit)))
          (:idle
           ()
           (test-render-clear renderer)
           (test-render-hello renderer)
           (test-render-lines renderer)
           (test-render-points renderer)
           (test-render-rect renderer)
           (test-render-rects renderer)
           (test-render-fill-rect renderer)
           (test-render-fill-rects renderer)
           (sdl3:render-present renderer)
	   (sdl3:delay 33))
          (:quit () t))))))

#+nil
(renderer-test)